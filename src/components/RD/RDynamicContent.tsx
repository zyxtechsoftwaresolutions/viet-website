...
const RDynamicContent: FC<RDynamicContentProps> = ({ activeSection, content }) => {
  ...
  const [selectedGalleryDepartment, setSelectedGalleryDepartment] = useState('All');
  ...
  const galleryDepartments = useMemo(() => {
    const unique = Array.from(new Set(content.gallery.map((g) => g.department).filter(Boolean)));
    return ['All', ...unique];
  }, [content.gallery]);

  const galleryFiltered = useMemo(() => {
    if (selectedGalleryDepartment === 'All') return content.gallery;
    return content.gallery.filter((g) => g.department === selectedGalleryDepartment);
  }, [content.gallery, selectedGalleryDepartment]);
      ...
      {activeSection === 'gallery' && (
        <div>
          <SectionHeading
            label="R&D Works"
            title="Gallery"
            description="All uploaded R&D work photos. Filter by department to quickly find relevant activities."
          />

          <div className="mb-6 flex flex-wrap gap-2">
            {galleryDepartments.map((dept) => {
              const isActive = selectedGalleryDepartment === dept;
              return (
                <motion.button
                  key={dept}
                  type="button"
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.99 }}
                  onClick={() => setSelectedGalleryDepartment(dept)}
                  className={[
                    'px-3 py-2 rounded-full border text-xs font-semibold transition-colors whitespace-nowrap',
                    isActive
                      ? 'bg-primary/10 border-primary/30 text-slate-900'
          ...
          {content.gallery.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-slate-600">
              <p className="text-sm md:text-base">No R&amp;D gallery images yet.</p>
              <p className="text-xs md:text-sm mt-2 text-slate-500">Add photos from Admin → Research &amp; Development.</p>
            </div>
          ) : (
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-5">
              {galleryFiltered.map((item, i) => (
                <motion.figure
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: Math.min(i * 0.02, 0.25), duration: 0.45 }}
                  className="break-inside-avoid mb-5 group relative rounded-3xl overflow-hidden border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow"
                >
                  ...
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      {item.department && (
                        <Badge className="bg-primary/95 text-white border-0 text-[10px] mb-2">
                          {item.department}
                        </Badge>
                      )}
                      {item.title && <p className="text-sm font-semibold text-white">{item.title}</p>}
                      {!item.title && <p className="text-sm font-semibold text-white">R&amp;D Work</p>}
                    </div>
                  </div>
                </motion.figure>
              ))}
