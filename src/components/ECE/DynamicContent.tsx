...
const DynamicContent: React.FC<DynamicContentProps> = ({ activeSection, galleryImages, openImageModal }) => {
  ...
  // Fetch faculty and HODs from API for ECE department
  useEffect(() => {
    ...
    const calculateStats = (apiFaculty: FacultyMember[], hodsData: FacultyMember[]) => {
      ...
      const phdHolders = uniqueFaculty.filter(f => {
        ...
        // More comprehensive Ph.D detection
    ...
    const fetchFaculty = async () => {
      try {
        ...
        // Filter faculty for ECE departments (both Diploma and UG)
        const eceFaculty = allFaculty.filter((f: FacultyMember) => {
          const dept = (f.department || '').toLowerCase().trim();
          return (
            dept.includes('diploma - ece') ||
            dept.includes('engineering ug - electronics and communication') ||
            dept.includes('engineering ug - ece') ||
            dept.includes('electronics and communication') ||
            dept.includes('ece') ||
            dept === 'electronics and communication engineering (ece)' ||
            dept.includes('electronics & communication')
          );
        });
        const eceHODs = allHODs.filter((h: FacultyMember) => {
        ...
        setFacultyFromAPI(eceFaculty);
        setHODs(eceHODs);
        // Calculate statistics after fetching
        calculateStats(eceFaculty, eceHODs);
      } catch (error) {
          ...
          <>
            ...
            <div className="prose prose-lg max-w-none">
              <p className="text-gray-700 mb-6">
                The Department of Electronics and Communication Engineering offers an undergraduate program with an intake of 180 students and a postgraduate program, M.Tech in VLSI & ES specialization. The department has well-qualified and experienced faculty with excellent infrastructural and well-equipped lab facilities.
              </p>
              <p className="text-gray-700 mb-6">
                The department focuses on areas such as Communication, Nanotechnology, Signal Processing, Image Processing, VLSI Design, Antennas, and Wave Propagation. The department contributes to research by encouraging faculty in various fields and provides Wi-Fi-enabled classrooms, laboratories, and staff rooms.
              </p>
            </div>
            <div className="bg-green-50 p-6 rounded-lg">
              ...
              <div className="space-y-3">
                ...
                <p className="text-green-700"><strong>Dr. Jeevanarao Batakala</strong></p>
                <div className="flex items-center gap-2 text-green-700">
                  <Phone className="w-4 h-4" />
                  <span>+91-9182854272</span>
                </div>
                <div className="flex items-center gap-2 text-green-700">
                  <Mail className="w-4 h-4" />
                  <span>ecehod@viet.edu.in</span>
                </div>
              </div>
            </div>
          </>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-blue-50 p-6 rounded-lg">
                ...
                <p className="text-gray-700 italic">
                  "The Electronics and Communication Engineering Department wants to be recognized as the leading Department in the state in terms of education and research and to be recognized nationally as a leader in at least three areas of technical expertise."
                </p>
              </div>
              <div className="bg-green-50 p-6 rounded-lg">
                ...
                <ul className="space-y-3 text-gray-700">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Provide education for students to compete internationally and produce creative solutions to society's needs.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <ChevronRight className="w-4 h-4 mt-1 text-green-600" />
                    <span>Offer quality education to make students entrepreneurs and employable.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <ChevronRight className="w-4 h-4 mt-1 text-green-600" />
                    <span>Continuously upgrade techniques to achieve excellence globally.</span>
                  </li>
                </ul>
              </div>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                      <tr>
                        <td className="border border-gray-300 px-4 py-2">1</td>
                        <td className="border border-gray-300 px-4 py-2">VLSI & EMBEDDED SYSTEMS</td>
          ...
          <>
            ...
            <div className="bg-blue-50 p-6 rounded-lg mb-8">
              ...
              <p className="text-gray-700 mb-4">
                "The Department of Electronics & Communication Engineering has highly qualified and efficient staff. The main aim of the department is not only to provide a strong base but also to give exposure about the latest technological trends required to survive in the Industrial Sector. Technologies like VLSI, Embedded Systems, and Image Processing help mold quality engineers for today's competitive environment."
              </p>
              <p className="text-gray-700 font-semibold">Dr. Jeevanarao Batakala</p>
              <p className="text-gray-600 text-sm">15 years of teaching and industrial experience</p>
            </div>
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-6">
                <div className="bg-white border rounded-lg p-6 shadow-sm">
                  <button 
                    onClick={() => selectProgram('B.Tech ECE')}
                    className={`w-full text-left p-4 rounded-lg transition-colors mb-4 ${
                      selectedProgram === 'B.Tech ECE' ? 'bg-primary text-white' : 'bg-primary/10 hover:bg-primary/20 text-primary'
                    }`}
                  >
                    <h4 className="text-lg font-semibold flex items-center justify-between">
                      B.Tech Electronics & Communication Engineering
                      <ChevronDown className={`w-5 h-5 transition-transform ${selectedProgram === 'B.Tech ECE' ? 'rotate-180' : ''}`} />
                    </h4>
                  </button>
                  {selectedProgram === 'B.Tech ECE' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {['R19', 'R20', 'R23', 'VR24'].map((reg) => (
                        <button 
                          key={reg}
                          onClick={() => downloadSyllabusPDF('B.Tech ECE', reg)}
            ...
            <div className="mb-8">
              ...
              <div className="space-y-6">
                <div className="bg-white border rounded-lg p-6 shadow-sm">
                  <button 
                    onClick={() => selectProgram('M.Tech VLSI & ES')}
                    className={`w-full text-left p-4 rounded-lg transition-colors mb-4 ${
                      selectedProgram === 'M.Tech VLSI & ES' ? 'bg-primary text-white' : 'bg-primary/10 hover:bg-primary/20 text-primary'
                    }`}
                  >
                    <h4 className="text-lg font-semibold flex items-center justify-between">
                      M.Tech VLSI & Embedded Systems
                      <ChevronDown className={`w-5 h-5 transition-transform ${selectedProgram === 'M.Tech VLSI & ES' ? 'rotate-180' : ''}`} />
                    </h4>
                  </button>
                  {selectedProgram === 'M.Tech VLSI & ES' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {['R18', 'R20', 'R23', 'R24'].map((reg) => (
                        <button 
                          key={reg}
                          onClick={() => downloadSyllabusPDF('M.Tech VLSI & ES', reg)}
                          className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors text-blue-800"
                        >
                          <FileText className="w-5 h-5" />
                          <span className="font-medium">{reg}</span>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">Graduates will demonstrate technical competence in electronics and communication engineering to solve complex problems.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">Graduates will exhibit professional growth through continuous learning and leadership roles.</p>
                    </div>
                  </div>
                </div>
              </div>
