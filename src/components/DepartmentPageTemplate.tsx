...
import { AdaptiveSectionNav } from '@/components/AdaptiveSectionNav';
...
const DepartmentPageTemplate: React.FC<DepartmentPageTemplateProps> = ({
  ...
  const [teachingFacultyHods, setTeachingFacultyHods] = useState<any[]>([]);
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-sm">
        <div className="w-full px-4 md:px-8 lg:px-12">
          <AdaptiveSectionNav itemCount={visibleNavSections.length} className="py-2.5">
            {visibleNavSections.map(({ id, label }) => (
              <button
                key={id}
                onClick={() => scrollTo(id)}
                className={cn(
                  'flex-shrink-0 px-2 py-2 rounded-md text-xs font-medium transition-colors whitespace-nowrap',
