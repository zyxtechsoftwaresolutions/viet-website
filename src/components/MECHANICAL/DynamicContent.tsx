...
const DynamicContent: React.FC<DynamicContentProps> = ({ activeSection, galleryImages, openImageModal }) => {
  ...
  // Fetch faculty and HODs from API for Mechanical Engineering department
  useEffect(() => {
    ...
    const fetchFaculty = async () => {
      try {
        ...
        const mechFaculty = allFaculty.filter((f: FacultyMember) => {
          const dept = (f.department || '').toLowerCase().trim();
          return (
            dept.includes('diploma - mechanical') ||
            dept.includes('engineering ug - mechanical') ||
            dept.includes('mechanical engineering') || dept.includes('mech') ||
            dept === 'mechanical engineering (mech)' || dept === 'mechanical engineering'
          );
        });
        const mechHODs = allHODs.filter((h: FacultyMember) => {
        ...
        setFacultyFromAPI(mechFaculty);
        setHODs(mechHODs);
        calculateStats(mechFaculty, mechHODs);
      } catch (error) {
          ...
          <>
            ...
            <div className="prose prose-lg max-w-none">
              <p className="text-gray-700 mb-6">
                The Department of Mechanical Engineering was established in 2011 with an initial intake of 60 students, which increased to 240 in 2014, and currently stands at 120 from the academic year 2023–2024. The department offers two postgraduate courses: M.Tech in CAD/CAM and Thermal Engineering, each with an intake of 18 students.
              </p>
              <p className="text-gray-700 mb-6">
                Accredited by NAAC with an "A" Grade, the department emphasizes academic excellence, research, and consultancy activities. Faculty and students are encouraged to enhance their knowledge through NPTEL courses and have established MoUs with various industry research organizations.
              </p>
              <p className="text-gray-700 mb-6">
                The department boasts well-equipped laboratories, including Workshop, Fluid Mechanics, Strength of Materials, Instrumentation, Metallurgy, CAD Simulation, Thermal Engineering, and Refrigeration and Air Conditioning labs.
              </p>
            </div>
            <div className="bg-green-50 p-6 rounded-lg">
              ...
              <div className="space-y-3">
                ...
                <p className="text-green-700"><strong>Department of Mechanical Engineering</strong></p>
              ...
              <div className="bg-green-50 p-6 rounded-lg">
                ...
                <ul className="space-y-3 text-gray-700">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Providing facilities and an environment conducive to high-quality education, grounding students in fundamental engineering principles and preparing them for diverse careers.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <ChevronRight className="w-4 h-4 mt-1 text-green-600" />
                    <span>Engaging in academic and scholarly activities to strengthen the department's regional, national, and international reputation.</span>
                  </li>
                </ul>
              </div>
            </div>
                  ...
                  <table className="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="border border-gray-300 px-4 py-2">THERMAL ENGINEERING</td>
                        <td className="border border-gray-300 px-4 py-2">18</td>
                        <td className="border border-gray-300 px-4 py-2">50,000</td>
                      </tr>
                    </tbody>
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-6">
                <div className="bg-white border rounded-lg p-6 shadow-sm">
                  <button 
                    onClick={() => selectProgram('B.Tech Mechanical Engineering')}
                    className={`w-full text-left p-4 rounded-lg transition-colors mb-4 ${
                      selectedProgram === 'B.Tech Mechanical Engineering' ? 'bg-primary text-white' : 'bg-primary/10 hover:bg-primary/20 text-primary'
                    }`}
                  >
                    <h4 className="text-lg font-semibold flex items-center justify-between">
                      B.Tech Mechanical Engineering
                      <ChevronDown className={`w-5 h-5 transition-transform ${selectedProgram === 'B.Tech Mechanical Engineering' ? 'rotate-180' : ''}`} />
                    </h4>
                  </button>
                  {selectedProgram === 'B.Tech Mechanical Engineering' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {['R19', 'R20', 'R23', 'VR24'].map((reg) => (
                        <button 
                          key={reg}
                          onClick={() => downloadSyllabusPDF('B.Tech Mechanical Engineering', reg)}
            ...
            <div className="mb-8">
              ...
              <div className="space-y-6">
                <div className="bg-white border rounded-lg p-6 shadow-sm">
                  <button 
                    onClick={() => selectProgram('M.Tech CAD/CAM')}
                    className={`w-full text-left p-4 rounded-lg transition-colors mb-4 ${
                      selectedProgram === 'M.Tech CAD/CAM' ? 'bg-primary text-white' : 'bg-primary/10 hover:bg-primary/20 text-primary'
                    }`}
                  >
                    <h4 className="text-lg font-semibold flex items-center justify-between">
                      M.Tech CAD/CAM
                      <ChevronDown className={`w-5 h-5 transition-transform ${selectedProgram === 'M.Tech CAD/CAM' ? 'rotate-180' : ''}`} />
                    </h4>
                  </button>
                  {selectedProgram === 'M.Tech CAD/CAM' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {['R18', 'R20', 'R23', 'R24'].map((reg) => (
                        <button 
                          key={reg}
                          onClick={() => downloadSyllabusPDF('M.Tech CAD/CAM', reg)}
                          className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors text-blue-800"
                        >
                          <FileText className="w-5 h-5" />
                          <span className="font-medium">{reg}</span>
                ...
                <div className="bg-white border rounded-lg p-6 shadow-sm">
                  <button 
                    onClick={() => selectProgram('M.Tech Thermal Engineering')}
                    className={`w-full text-left p-4 rounded-lg transition-colors mb-4 ${
                      selectedProgram === 'M.Tech Thermal Engineering' ? 'bg-primary text-white' : 'bg-primary/10 hover:bg-primary/20 text-primary'
                    }`}
                  >
                    <h4 className="text-lg font-semibold flex items-center justify-between">
                      M.Tech Thermal Engineering
                      <ChevronDown className={`w-5 h-5 transition-transform ${selectedProgram === 'M.Tech Thermal Engineering' ? 'rotate-180' : ''}`} />
                    </h4>
                  </button>
                  {selectedProgram === 'M.Tech Thermal Engineering' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {['R18', 'R20', 'R23', 'R24'].map((reg) => (
                        <button 
                          key={reg}
                          onClick={() => downloadSyllabusPDF('M.Tech Thermal Engineering', reg)}
                          className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors text-blue-800"
                        >
                          <FileText className="w-5 h-5" />
                          <span className="font-medium">{reg}</span>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">Graduates will demonstrate technical competence in mechanical engineering to solve complex problems in design, manufacturing, and thermal systems.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">Graduates will exhibit professional growth through continuous learning and leadership roles in the mechanical engineering profession.</p>
                    </div>
                  </div>
                </div>
              </div>
