...
export type NssIconItem = { title: string; icon: string };
export type NssContactColumn = { title: string; lines: string[]; email?: string };

export type NssContent = {
  hero: FacilityHero;
  about: { label: string; title: string; paragraphs: string[] };
  stats: { value: string; label: string }[];
  objectives: NssIconItem[];
  activities: NssIconItem[];
  specialCamp: {
    title: string;
    description: string;
    stats: { value: string; label: string }[];
  ...
  contact: { columns: NssContactColumn[] };
};

export const DEFAULT_NSS_CONTENT: NssContent = {
  hero: {
...
function normalizeIconItems(v: unknown, fallback: NssIconItem[]): NssIconItem[] {
...
export function normalizeNssContent(raw: unknown): NssContent {
  const d = DEFAULT_NSS_CONTENT;
  const c = raw && typeof raw === 'object' ? (raw as Record<string, unknown>) : {};
  const about = c.about && typeof c.about === 'object' ? (c.about as Record<string, unknown>) : {};
  const specialCamp =
    c.specialCamp && typeof c.specialCamp === 'object' ? (c.specialCamp as Record<string, unknown>) : {};
  const getInvolved =
    c.getInvolved && typeof c.getInvolved === 'object' ? (c.getInvolved as Record<string, unknown>) : {};
  ...
  const columns: NssContactColumn[] = (columnsRaw ?? d.contact.columns).map((col: any, i: number) => ({
  ...
  return {
    ...
    stats: asStatPairs(c.stats, d.stats),
    objectives: normalizeIconItems(c.objectives, d.objectives),
    activities: normalizeIconItems(c.activities, d.activities),
    specialCamp: {
      title: asString(specialCamp.title, d.specialCamp.title),
      description: asString(specialCamp.description, d.specialCamp.description),
      stats: asStatPairs(specialCamp.stats, d.specialCamp.stats),
    },
    getInvolved: {
      label: asString(getInvolved.label, d.getInvolved.label),
      title: asString(getInvolved.title, d.getInvolved.title),
      description: asString(getInvolved.description, d.getInvolved.description),
      eligibility: asStringArray(getInvolved.eligibility, d.getInvolved.eligibility),
      benefits: asStringArray(getInvolved.benefits, d.getInvolved.benefits),
      ctaText: asString(getInvolved.ctaText, d.getInvolved.ctaText),
      ctaHref: asString(getInvolved.ctaHref, d.getInvolved.ctaHref),
    },
    contact: { columns },
  };
}
